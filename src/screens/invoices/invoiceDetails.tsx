    ...
    return (
        <View>
            {items.map((item, index) => (
                <View key={index} style={[{ height: 80, width: '90%', backgroundColor: colors.whiteTwo, borderWidth: 1, borderRadius: 8, borderColor: colors.greyFour }]}>
                    <View style={[flexRow, justifyBetween, alignItemCenter]}>
                        ...
                        <View style={[commonStyles.smallCard, mr10, mt10, alignItemCenter, { backgroundColor: colors.redSix }]}>
                            ...
                            <Text style={[commonStyles.h12RedTwo500, mh10]} >${parseFloat(item.amount).toFixed(2)}</Text>
                        </View>
                    </View>
                    <View style={[flexRow, justifyBetween, alignItemCenter, mh15, mt5]}>
                        <View>
                            <Text style={[commonStyles.h12blackTwo400]}>Unit</Text>
                            <Text style={[commonStyles.h12BlackOne600]}>{item.units}</Text>
                        </View>
                        <View>
                            <Text style={[commonStyles.h12blackTwo400]}>Quantity</Text>
                            <Text style={[commonStyles.h12BlackOne600]}>{item.quantity}</Text>
                        </View>
                        <View>
                            <Text style={[commonStyles.h12blackTwo400]}>Rate</Text>
                            <Text style={[commonStyles.h12BlackOne600]}>${parseFloat(item.rate).toFixed(2)}</Text>
                        </View>
                        <View>
                            <Text style={[commonStyles.h12blackTwo400]}>Discount</Text>
                            <Text style={[commonStyles.h12BlackOne600]}>${parseFloat(item.discount).toFixed(2)}</Text>
                        </View>
                    </View>
                </View>
            ))}
...
const InvoiceDetailsScreen: React.FC<InvoiceDetailsProps> = ({ route }) => {
    ...
    function handleActionClick(item?: any) {
        console.log('handleActionClick', item);
        setShowModal(true);
        setText(item?.name);
    }
    ...
    function HandleAction() {
        ...
        if (text === 'Clone' || text === 'Convert') {
            console.log('cloning', invoiceDetails);
            postMethod(
                ApiUrl.addInvoice + `/${invoiceDetails?._id}/${text === 'Clone' ? 'clone' : 'convertsalesreturn'}`,
                {},
            ...
            getMethod(
                ApiUrl.addInvoice + `/pdfCreate?invoiceId=${invoiceDetails?._id}} `,
                (success: { code: number; message: any; }) => {
                    if (success.code === 200) {
                        toast.show(`Invoice ${text} Success`, { type: "normal", placement: "bottom", duration: 4000, animationType: "slide-in", });
                        navigation.navigate(screenName.InvoiceScreen as never)
    ...
    const handleEditInvoice = async (invoiceDetails: any) => {
        console.log('handleEditInvoice', invoiceDetails);
        const editInvoice = await handleGetInvoiceData(invoiceDetails);
        console.log('editInvoice', editInvoice);
        navigation.navigate(screenName.AddInvoiceScreen, { isAddedProductAvailable: false, invoiceDetails: editInvoice?.invoice_details } as any)
    }
    return (
        <Fragment>
            ...
            <SafeAreaView style={getMainContainerStyle(false)}>
                <View style={[flex1]}>
                    <View style={[mh15, mv10]}>
                        <TopHeader headerText={labels.invoiceDetails} />
                    </View>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        <View style={[flex1, mt10, mh10, mb10]}>
                            ...
                            <View style={[flexRow, justifyBetween, alignItemCenter, mv10]}>
                                <View>
                                    <Text style={[commonStyles.h18BlackOne700, {}]}>Invoice No</Text>
                                    <Text style={[commonStyles.h12RedTwo500]}>#INV{(invoiceDetails.invoiceNumber).replace(/\s/g, '')}</Text>
                                </View>
                                <View style={[flexRow, justifyBetween, alignItemCenter]}>
                                    <TouchableOpacity onPress={() => handleEditInvoice(invoiceDetails)} style={[{ height: 30, width: 30, backgroundColor: colors.greyOne, borderRadius: 15 }, alignItemCenter, justifyCenter]}>
                                    ...
                                    <View style={[{ padding: 5, backgroundColor: colors.green, borderRadius: 5 }, mr5, alignItemCenter, justifyCenter, flexRow, ml15]}>
                                        <View style={[{ height: 6, width: 6, borderRadius: 10, backgroundColor: colors.white, }]} />
                                        <Text style={[commonStyles.h12white600]} style={[ml5]}> {invoiceDetails.status} </Text>
                                    </View>
                                </View>
                            </View>
                            <View style={[{ paddingVertical: 10, width: '100%', backgroundColor: colors.white, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.50, shadowRadius: 3.84, elevation: 2, borderRadius: 8 }, justifyCenter]}>
                                <View style={[flexRow, justifyBetween, alignItemCenter,]}>
                                    <View style={[ml10]}>
                                        <Text style={commonStyles.h14blackOne600}>Issue Date</Text>
                                        <View style={[{ height: 40, width: DevWidth / 2.3, backgroundColor: colors.greyOne, borderRadius: 5 }, alignItemStart, justifyCenter, mt5]}>
                                            <Text style={[commonStyles.h12blackTwo400, ml10]}>{moment(invoiceDetails.invoiceDate).format('DD MMM YYYY')}</Text>
                                        </View>
                                    </View>
                                    <View style={[mr10]}>
                                        <Text style={commonStyles.h14blackOne600}>Due Date</Text>
                                        <View style={[{ height: 40, width: DevWidth / 2.3, backgroundColor: colors.greyOne, borderRadius: 5 }, alignItemStart, justifyCenter, mt5]}>
                                            <Text style={[commonStyles.h12blackTwo400, ml10]}>{moment(invoiceDetails.dueDate).format('DD MMM YYYY')}</Text>
                                        </View>
                                    </View>
                                </View>
                                <View style={[flexRow, alignItemCenter, justifyBetween, mh10, mv10]}>
                                    <View style={[flex1]}>
                                        <Text style={commonStyles.h14blackOne600}>Invoice To</Text>
                                    </View>
                                    <View style={{ width: 10 }} />
                                    <View style={[flex1]}>
                            ...
                            <View style={[flexRow, justifyAround]}>
                                {
                                    quotationBottomBarData.map((item) => {
                                        return (
                                            <TouchableOpacity onPress={() => handleActionClick(item)} key={item.id}>
                                                ...
                                                <Text style={[commonStyles.h14blackTwo500]} style={[mt5]}>{item.name}</Text>
                                            </TouchableOpacity>
                                        )
                                    })
                                }

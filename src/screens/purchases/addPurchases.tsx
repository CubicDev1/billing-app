...
import { FlatList, Image, Platform, SafeAreaView, ScrollView, Text, ToastAndroid, TouchableOpacity, View } from 'react-native';
...
import { alignItemCenter, alignSelfCenter, flex1, flexRow, justifyBetween, justifyCenter, justifyStart, m10, mb5, mh10, mh15, ml10, ml15, mr10, mr5, mt10, mt15, mt5, mv10, mv15, mv5, p10 } from '../../utils/theme/commonStyles';
...
import { CalculateDiscount, CalculatePrice, CalculateTaxTotal, CalculationTotal } from '../../utils/Calculation';
import { convertToFormData } from '../../utils/FormDataFunc';
import { AddPurchaseData, UpdatePurchaseData } from '../../userCall/ApiUser';
...
const AddPurchases = ({ route }: any) => {
    ...
    const [selectedDateType, setSelectedDateType] = useState<'purchaseDate' | null>(null);
    ...
    const getEditVendorData = () => {
        // setInvNo(vendorDetails?.invoiceNumber.replace(/\s+/g, ''))
        // console.log('invoiceDetails', vendorDetails?.customerId?.name);

        handleDropdownChange(vendorDetails.vendorId)
        setValue('referenceNo', vendorDetails?.referenceNo)
        setValue('purchaseDate', moment(vendorDetails?.purchaseDate).format('YYYY-MM-DD'))
        // setValue('payment_method', vendorDetails?.payment_method)
        ...
        setTermsCondition(vendorDetails?.terms_conditions ?? vendorDetails.termsAndCondition);
        ...
        setValue('supplierInvoiceSerialNumber', vendorDetails?.supplierInvoiceSerialNumber)
    ...
    const handleDateConfirm = (date: any) => {
        ...
        if (selectedDateType === 'purchaseDate') {
    ...
    const showDatePickerModal = (type: 'purchaseDate') => {
    ...
    const getUpdateBank = () => {
        console.log("get update bank");

        getMethod(
            ApiUrl.dropDownBank,
            async success => {
                if (success.code === 200) {
                    await dispatch({ type: TYPES.BANK_LIST, payload: success.data });
                    ...
                    <View style={[flex1, { backgroundColor: colors.whiteTwo }]}>
                         <KeyboardAwareScrollView
                            ...
                            <View style={[flex1, mt10, mh10]}>
                                <View style={mv15}>
                                    <TopHeader headerText={labels.addPurchase} />
                                </View>
                                <View style={[mv10]}>
                                    ...
                                    <TouchableOpacity onPress={() => showDatePickerModal('purchaseDate')} style={[styles.input, flexRow]}>
                                        <Controller
                                            name={'purchaseDate'}
                                            control={control}
                                            render={({ field: { onChange, value } }) => (
                                    ...
                                    <View style={[flexRow, mt15]}>
                                       <Text style={[commonStyles.h14blackOne600,mv5]} >{labels.supplierInvoiceSerialNumber}</Text>
                                        <Text style={[commonStyles.h14Danger400]}> *</Text>
                                    </View>
                                    <View style={styles.input}>
                                        <Controller
                                            ...
                                            render={({ field: { onChange, value } }) => (
                                                <CustomTextInput
                                                    placeholder={labels.supplierInvoiceSerialNumber}
                                                    value={value}
                                                    onChangeText={onChange}
                                                    textColor={colors.black}
                                                    // error={errors['supplierInvoiceSerialNumber']?.message}
                                                    keyboardType='number-pad'
                                                    maxLength={30}
                                                />
                                            )}
                                            rules={{
                                                required: requiredValidation("supplierInvoiceSerialNumber"),
                                            }}
                                        />
                                    </View>
                                    {errors.supplierInvoiceSerialNumber && (
                                        <Text style={{ color: colors.danger, marginTop: 2, marginLeft: 10 }}>
                                            {errors.supplierInvoiceSerialNumber.message}
                                        </Text>
                                    )}
                                </View>
                                {

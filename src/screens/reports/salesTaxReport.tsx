...
import { FlatList, ScrollView, StyleSheet, Text, TouchableNativeFeedback, View } from 'react-native';
...
const SalesReport = (props: SalesReportProps) => {
    ...
    useEffect(() => {
        const formattedData = salesList.map(expense => ({
            value: getTaxAmount(expense.items),
    ...
    const getTaxAmount = (items: any) => {
        let taxTotal = 0;
        items.map((item: any) => {
            var taxInfo = JSON.parse(item.taxInfo);
            const sellingPrice = parseFloat(item?.rate) * parseFloat(item?.quantity ?? 0);
            const discountValue = item.discountType == "3" ? parseFloat(item.discount) :
                (sellingPrice * parseFloat(item?.discount ?? 0)) / 100;
            const taxRate = parseFloat(taxInfo.taxRate);
                                 ...
                                 <TouchableOpacity style={[commonStyles.mainListCard]} disabled>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', }}>
                                        <View style={{ flexDirection: 'row' }}>
                                            <View style={{ alignSelf: 'center', marginHorizontal: 8 }}>
                                               ...
                                               <Text style={[commonStyles.h10Primary600]}>{report.customerId._id}</Text>
                                            </View>
                                        </View>
                                        <View style={{ alignSelf: 'center' }}>
                                            <Text style={[commonStyles.h12blackTwo600]}>{labels.totalAmnt}</Text>
                                           <Text style={[commonStyles.h14blackOne600,alignSelfCenter]} >{currencySymbol}{report.TotalAmount}</Text>
                                        </View>
                                    </View>
                                    ...
                                    <View style={{ backgroundColor: colors.white4, padding: 10, borderRadius: 10 }}>
                                        <View style={[flexRow, justifyBetween]}>
                                            ...
                                            <View>
                                                <Text style={[commonStyles.h12blackTwo600]}>{labels.discount}</Text>
                                               <Text style={commonStyles.h14blackOne600}>{currencySymbol}{report.totalDiscount}</Text>
                                            </View>
                                            <View>
                                                <Text style={[commonStyles.h12blackTwo600]}>{labels.taxAmount}</Text>
                                               <Text style={commonStyles.h14blackOne600}>{currencySymbol}{getTaxAmount(report.items)}</Text>
                                            </View>
                                            {/* <View>
                                                <Text style={[commonStyles.h12blackTwo600]}>{labels.payments}</Text>
                                               <Text style={commonStyles.h14blackOne600}>{report.payment_method}</Text>
                                            </View> */}
                                        </View>
                                    </View>
                                </TouchableOpacity>

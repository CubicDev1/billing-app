...
const SalesReturnDetails = ({ route }: any) => {
    // Extract invoiceDetails from route.params
    const [invoiceDetails, setViewDetailsData] = useState<SalesReturnDetails | null>(null);
    ...
    const ViewPurchaseData = () => {
        try {
            getMethod(
                ApiUrl.viewSalesReturn + route.params.itemId,
                success => {
                    if (success.code === 200) {
            ...
            <SafeAreaView style={getMainContainerStyle(false)}>
                <View style={[flex1]}>
                    <View style={[mh15, mv10]}>
                        <TopHeader headerText={labels.salesReturnDetails} />
                    </View>
                                ...
                                <View>
                                    <View style={[{ paddingVertical: 10, width: '100%', backgroundColor: colors.white, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.50, shadowRadius: 3.84, elevation: 2, borderRadius: 8 }, justifyCenter]}>
                                        <View style={[flexRow, justifyBetween, alignItemCenter,]}>
                                            ...
                                            <View style={[mr10]}>
                                               <Text style={commonStyles.h14blackOne600}>Credit Note No</Text>
                                                <View style={[{ height: 40, width: DevWidth / 2.3, backgroundColor: colors.greyOne, borderRadius: 5 }, alignItemStart, justifyCenter, mt5]}>
                                                    <Text style={[commonStyles.h12blackTwo400,ml10]} >{invoiceDetails.credit_note_id}</Text>
                                                </View>
                                            </View>
                                        </View>
                                        <View style={[flexRow, alignItemCenter, justifyBetween, mh10, mv10]}>
                                            <View style={[flex1]}>
                                               <Text style={commonStyles.h14blackOne600}>Sales Return To</Text>
                                            </View>
                                            <View style={{ width: 10 }} />
                                            <View style={[flex1]}>
                                        ...
                                        <View style={[flexRow, alignItemCenter, justifyBetween, mh10]}>
                                            ...
                                            <View style={[{
                                                ...
                                                <Text style={[commonStyles.h12blackTwo400]} >{invoiceDetails.customerInfo?.name ?? "N/A"}</Text>
                                                <Text style={[commonStyles.h12blackTwo400]} >{invoiceDetails?.customerInfo?.email ?? "N/A"}</Text>
                                                <Text style={[commonStyles.h12blackTwo400]} >{invoiceDetails?.customerInfo?.phone ?? "N/A"}</Text>
                                            </View>
                                            ...
                                            {invoiceDetails?.items.map((item, index) => (
                                                <View style={{ width: '100%' }}>
                                                    <View key={index} style={[{ backgroundColor: colors.whiteTwo, borderWidth: 1, borderRadius: 8, borderColor: colors.greyFour }]}>
                                                        <View style={[flexRow, justifyBetween, alignItemCenter]}>
                                                            {/*<Text style={commonStyles.h14blackOne600} style={[ml10]}>{item.name}</Text> */}
                                                           <Text style={[commonStyles.h14blackOne600,ml10]}>{ProductList.find(product => product._id === item.productId)?.name ?? "N/A"}</Text>
                                    ...
                                    <View style={[flexRow, alignItemCenter, justifyBetween, mh10]}>
                                        ...
                                        <View style={[{
                                            ...
                                            <Text style={[,commonStyles.h12BlackOne600,mt5]}>{invoiceDetails?.notes}</Text>
                                        </View>
                                        ...
                                        <View style={[{
                                            ...
                                            <View style={{ width: '100%' }}>
                                            <Text style={[,commonStyles.h12BlackOne600,mt5]}>{invoiceDetails?.termsAndCondition}</Text>
                                            </View>
                                        </View>
                                    </View>
                                    <View style={[mv10, { alignItems: 'flex-end' }]}>
                                       <Text style={commonStyles.h14blackOne600} >{signName ?? 'Signature'}</Text>
                                        {correctUrl != null &&
                                            <View style={{}}>
